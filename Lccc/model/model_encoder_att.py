...
class AttentiveEncoder1(nn.Module):
    ...
    def change_detection(self, img1_list, img2_list, CC_feat_list = None):
        ...
        # fused = torch.cat(img_fus_list, dim=1) # Switch to a U-Net-style variant?
        # Added
        cd_pred_features = img_fus
        seg = self.to_seg(img_fus)
        return seg, cd_pred_features
    ...
    def forward(self, img1_list, img2_list):
        ...
        # detection
        seg, cd_pred_features = self.change_detection(CD_img1_list, CD_img2_list, None)

        return img1_cap, img2_cap, seg, cd_pred_features
