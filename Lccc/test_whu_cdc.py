...
from data.Dubai_CC import DubaiCCDataset
...
from model.model_t import MultiScaleAttentionSuppression, MultiScaleFeatureFusion
...
def main(args):
    ...
    print('module_ablation:', module_ablation)
    ...
    args.result_path = os.path.join(
        args.result_path,
        module_ablation + '_' + os.path.basename(snapshot_full_path).replace('.pth', '')
    )
    ...
    encoder_trans = AttentiveEncoder1(train_stage=None, n_layers=args.n_layers,
                                          feature_size=[args.feat_size, args.feat_size, args.encoder_dim],
                                          heads=args.n_heads, dropout=args.dropout)
    decoder = DecoderTransformerV1(encoder_dim=args.encoder_dim, feature_dim=args.feature_dim,
                                      vocab_size=len(word_vocab), max_lengths=args.max_length,
                                      word_vocab=word_vocab, n_head=args.n_heads,
                                      n_layers=args.decoder_n_layers, dropout=args.dropout)
    Suppression_module = MultiScaleAttentionSuppression(t1_channels=[64, 128, 320, 512],
                                                             t2_channels=512,
                                                             learnable_alpha=True, init_alpha=0.5)
    Complementary_module = MultiScaleFeatureFusion()

    encoder.load_state_dict(checkpoint['encoder_dict'])
    encoder_trans.load_state_dict(checkpoint['encoder_trans_dict'], strict=False)
    decoder.load_state_dict(checkpoint['decoder_dict'])
    if 'suppression_dict' not in checkpoint:
        raise KeyError('checkpoint does not contain suppression_dict for full model testing')
    Suppression_module.load_state_dict(checkpoint['suppression_dict'])
    if 'complementary_dict' not in checkpoint:
        raise KeyError('checkpoint does not contain complementary_dict for full model testing')
    Complementary_module.load_state_dict(checkpoint['complementary_dict'])
    ...
    Suppression_module.eval()
    Suppression_module.cuda()
    Complementary_module.eval()
    Complementary_module.cuda()
    ...
    elif args.data_name == 'DubaiCC':
        nochange_list = ["the scene is the same as before ", "there is no difference ",
                         "the two scenes seem identical ", "no change has occurred ",
                         "almost nothing has changed "]
        test_loader = data.DataLoader(
            DubaiCCDataset(args.data_folder, args.list_path, 'test',
                           token_folder=args.token_folder, vocab_file=args.vocab_file,
                           max_length=args.max_length, allow_unk=args.allow_unk),
            batch_size=args.test_batchsize, shuffle=True, num_workers=args.workers, pin_memory=True)
    elif args.data_name == 'WHU_CDC':
        nochange_list = ["the scene is the same as before ", "there is no difference ",
                         "the two scenes seem identical ", "no change has occurred ",
                         "almost nothing has changed "]
        test_loader = data.DataLoader(
            WHUCDCDataset(args.data_folder, args.list_path, 'test',
                          token_folder=args.token_folder, vocab_file=args.vocab_file,
                          max_length=args.max_length, allow_unk=args.allow_unk),
            batch_size=args.test_batchsize, shuffle=False, num_workers=args.workers, pin_memory=True)
    ...
    with torch.no_grad():
        for ind, (imgA, imgB, seg_label, token_all, token_all_len, _, _, name) in enumerate(
            ...
            feat1, feat2 = Suppression_module(feat1, feat2)
            feat1, feat2 = Complementary_module(feat1, feat2)
            feat1, feat2, seg_pre, _ = encoder_trans(feat1, feat2)

            seq = decoder.sample(feat1, feat2, k=1, seg_pre=seg_pre)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', default='./whu_CDC_dataset/images', help='folder with data files')
    parser.add_argument('--list_path', default='./data/WHU_CDC/', help='path of the data lists')
    parser.add_argument('--token_folder', default='./data/WHU_CDC/tokens/',
    ...
    parser.add_argument('--data_name', default="WHU_CDC", help='base name shared by data files.')
